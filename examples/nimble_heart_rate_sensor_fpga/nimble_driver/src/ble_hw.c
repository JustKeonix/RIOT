...
{
    return (int) 0;//NRF_RADIO->EVENTS_DEVMATCH;
}
...
{
    // This is called every time in NimBLE's RNG inititalisation routine,
    // so put some data into global RNG buff, imitating HWRNG ISR
    if(g_ble_rng_isr_cb != NULL) {
        for(int i = 0; i < MYNEWT_VAL(BLE_LL_RNG_BUFSIZE); i++){
            uint8_t rnum = ble_hw_rng_read();
            (*g_ble_rng_isr_cb)(rnum);
        }
    }
...
{
    // do nothing here, the HWRNG of STM32 in RIOT is userspace-only
