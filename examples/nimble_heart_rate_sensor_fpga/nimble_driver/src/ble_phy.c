...
static char phy_isr_stack[THREAD_STACKSIZE_DEFAULT];
static volatile int phy_isr_pid = 0;
static ztimer_t the_timer;

static msg_t phy_start_msg = {.content = {.value = 1}};
static msg_t tx_done_msg = {.content = {.value = 2}};
static msg_t rx_start_msg = {.content = {.value = 3}};
...
{
    // wait until status is "disabled" in a loop
    // make sure it is not run in an interrupt context
}
...
ble_phy_set_start_time(uint32_t cputime, uint8_t rem_usecs, bool tx)
{
    ...
    //printf("at cputime %lu, setting start time at %lu + %u usecs", now, cputime, rem_usecs);

    /* Store the cputime at which we set the RTC */
    g_ble_phy_data.phy_start_cputime = cputime;

    /* TODO: adjust time properly */
#if 0
    /*
    ...
    uint32_t delta = (cputime - now) * 1000 + rem_usecs;
    if(delta <= 0) {
        delta = 1; // не успеваем никак, но запустим хотя бы так
        // может быть, имеет смысл просто сразу кинуть сообщение потоку, а не возиться с таймером?
    }
    ztimer_set_msg(ZTIMER_USEC, &the_timer, delta, &phy_start_msg, phy_isr_pid);
...
ble_phy_set_start_now(void)
{
    ...
    // set an xtimer to fire "soon" (3 ms from now)?
    // maybe sending a message is better?
    ztimer_set_msg(ZTIMER_USEC, &the_timer, 3*1000, &phy_start_msg, phy_isr_pid);
    //msg_try_send(&timer_msg, phy_isr_stack_pid);
...
ble_phy_tx_end_isr(void)
{
    ...
    if (transition == BLE_PHY_TRANSITION_TX_RX) {
        ...
        /* TODO: see below an adjustment example */
        //puts("Schedule RX exactly T_IFS after TX end");
#if 0
...
#endif 
        ztimer_set_msg(ZTIMER_USEC, &the_timer, BLE_LL_IFS, &rx_start_msg, phy_isr_pid);
...
void *phy_isr_thread(void *arg) {
    (void) arg;

    /* Let's use a queue so messages are not lost here */
    msg_t msgq[4];
    ...
    while (1) {
        msg_t m;
        msg_receive(&m);
        // TODO: The following should be similar to ble_phy_isr

        if(m.content.value == 1) {
            // example: start transmission
            // at his moment, data should be ready
            puts("Now PHY may start!");

            // schedule a TX end for ourselves if it was a TX
            ztimer_set_msg(ZTIMER_USEC, &the_timer, 100, &tx_done_msg, phy_isr_pid);
        }

        if(m.content.value == 2) {
            // example: handle disable event when TX is over
            #if MYNEWT_VAL(BLE_LL_PA)
            //ble_ll_plna_pa_disable();
            #endif
            ble_phy_tx_end_isr();
        ...
        if(m.content.value == 3) {
            ble_ll_wfr_timer_exp(NULL); // we need this to get the advertising FSM running
            //for(int i = 0; i < c; i++) {
            //    printf("At %li, message %li received\n", events[i], types[i]);
            //}
            //c = 0;
        }
    }
...
ble_phy_init(void)
{
    ...
    // here we initialize a timer and a pseudo-ISR thread;
    // the thread is required to talk with external hardware through SPI
    // and similar interfaces in RIOT, so we signal it with messages from
    // actual ISRs (GPIO, ztimer callbacks, etc)
    if(phy_isr_pid == 0) {
        phy_isr_pid = thread_create(phy_isr_stack, sizeof(phy_isr_stack),
                0, // highest priority
...
ble_phy_tx(ble_phy_tx_pducb_t pducb, void *pducb_arg, uint8_t end_trans)
{
    ...
    puts("ble_phy_tx(), here we setup the PDU for transmission");
    ...
    // now we should transfer dptr into FPGA ASAP!
    // we are in interrupt context, so send a message to the thread and hope it goes well
#if 0
    uint8_t *dptr;
    uint8_t *pktptr;
    uint8_t payload_len;
